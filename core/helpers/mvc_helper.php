...
class MvcHelper {
	...
	public function admin_actions_cell( $controller, $object, $options = array() ) {
		...
		if ( $options['actions']['edit'] ) {
			...
			) . '" title="' . __( 'Edit', 'wpmvc' ) . ' ' . $encoded_object_name . '">' . __( 'Edit', 'wpmvc' ) . '</a>';
		}
		...
		if ( $options['actions']['delete'] ) {
			...
			) . '" title="' . __( 'Delete', 'wpmvc' ) . ' ' . $encoded_object_name . '" onclick="return confirm(&#039;' . __( 'Are you sure you want to delete', 'wpmvc' ) . ' ' . $encoded_object_name . '?&#039;);">' . __( 'Delete', 'wpmvc' ) . '</a>';
		}
