...
class MvcModel {
	...
	public function validate( $data ) {
		$errors = $this->validate_all_data( $data );
		if ( is_array( $errors ) ) {
			$messages = array();
			foreach ( $errors as $error ) {
				$messages[] = $error->get_html();
			}
			$this->validation_error = $messages;
		...
		foreach ( $objects as $key => $object ) {
			...
			if ( ! empty( $includes ) && $recursive != 0 ) {
				foreach ( $includes as $include_key => $include ) {
					...
					switch ( $association['type'] ) {
						case 'belongs_to':
							$associated_object                                 = $model->find_by_id(
								$object->{$association['foreign_key']}, array(
									'recursive' => $recursive,
								)
							);
	...
	protected function init_properties() {
		...
		foreach ( $this->associations as $association_name => $association ) {
			...
			} elseif ( in_array( $association['type'], array( 'has_many', 'has_and_belongs_to_many' ) ) ) {
