...
class MvcPostAdapter {
	...
	public function verify_settings( $model ) {
		...
		} elseif ( ! isset( $model->schema['post_id'] ) ) {
			MvcError::fatal(
				'To associate posts with ' . $model->name . ', its table needs to have a column named "post_id" of type BIGINT(20).  Please run the following SQL to add and index the column:
<pre>
